        ...
        <div className="content search-div" style={{ top: "0.8rem", position: "relative", zIndex: 10 }}>
          ...
          <div className="authen-content-frame">
            ...
            <div className="auth-tips-item">
              <p className="auth-tips">如何获取防伪编码：您可以在商品外包装上找到防伪贴纸，刮开涂层，获得4位防伪编码。</p>
            </div>

            <div className="auth-msg-item">
              <p className={`auth-search-result ${resultState === "idle" ? "c-none" : ""}`} style={{ display: resultState === "idle" ? "none" : "block" }}>
                查询结果
              </p>
              ...
              <div className="auth-result-frame">
                ...
                />
                <img
                  className={`c-w100p auth-img-ok ${resultState === "ok" ? "" : "c-none"}`}
                  src={okImage}
                  alt="ok"
                  style={{ display: resultState === "ok" ? "block" : "none" }}
                />
                <img
                  className={`c-w100p auth-img-warn ${resultState === "fail" ? "" : "c-none"}`}
                  src={warnImage}
                  alt="warn"
                  style={{ display: resultState === "fail" ? "block" : "none" }}
                />

                <span
                  className={`auth-result-fail ${resultState === "fail" ? "" : "c-none"}`}
                  style={{ display: resultState === "fail" ? "block" : "none" }}
                >
                ...
                <span
                  className={`auth-result-ok ${resultState === "ok" ? "" : "c-none"}`}
                  style={{ display: resultState === "ok" ? "block" : "none" }}
                >
                  查询有效，验证成功
                </span>
            ...
            <div className={`auth-info-item ${resultState === "ok" ? "" : "c-none"}`} style={{ display: resultState === "ok" ? "block" : "none" }}>
              <p className="auth-info-num c-none" style={{ display: "block" }}>
                溯源号码：<span className="auth-serinum">{traceCode}</span>
              </p>
              <p className="auth-product-name c-none" style={{ display: "block" }}>
                产品名称：<span className="auth-pro-name"> 标示品牌：龙骧  手袋 23-0419-117</span>
              </p>
              <p className="auth-comp-name c-none" style={{ display: "block" }}>
                企业名称：<span className="auth-com-name">中国检验认证集团上海有限公司（奢侈品鉴定）</span>
              </p>
            </div>
          </div>
        </div>
